...
import { Side, FleetType, Formation, BattleFleet } from 'kc-calculator'
...
const stringToFormation = (value: string, isCombinedFleet?: boolean) => {
  const formation = Formation.all.find(({ name }) => name === value)
...
const createEnemyBattleFleet = (enemy: TEnemyFleet) => {
  ...
  const fleetType = isCombinedFleet ? FleetType.Combined : FleetType.Single
